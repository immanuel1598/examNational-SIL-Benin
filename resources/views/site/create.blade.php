...
@section('content')
    <div class="container">
        <h1>Enregistrer des Courses</h1>
        <form action="{{ route('courses.store') }}" method="POST" >
            @csrf
            <div class="mb-3">
                <label for="point_depart" class="form-label">point_depart</label>
                <input type="text" class="form-control @error('point_depart') is-invalid @enderror" id="point_depart" name="point_depart" value="{{ @old('point_depart') }}">
                @error('point_depart')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            <div class="mb-3">
                <label for="point_arrivee" class="form-label">point_arrivee</label>
                <input class="form-control @error('point_arrivee') is-invalid @enderror" id="point_arrivee" name="point_arrivee" value="{{ @old('point_arrivee') }} " >
                @error('point_arrivee')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control @error('date_heure') is-invalid @enderror" id="date_heure" name="date_heure" value="{{ @old('date_heure') }}" >
                @error('date_heure')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
